"""
CLI entry point for inspectre.

Invoked by the ``inspectre`` console script installed by pip, or directly via
``python -m inspectre``.

Usage::

    inspectre -f <binary>
    inspectre --kernel <Image> [--disk <img>] [--dtb <dtb>]
    inspectre --script <script.py> [args ...]
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  inspectre -f hello.elf\n"
            "  inspectre --kernel Image --disk root.img\n"
            "  inspectre --script experiment.py --ipc 4\n"
        ),
    )
    ...
    if args.script:
        ...
        # The inspectre package is already importable (we're inside it).
