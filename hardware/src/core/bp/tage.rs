...
struct TageEntry {
    tag: u16,
    ctr: i8, // 3-bit counter (-4 to 3)
    u: u8,   // 2-bit useful counter
}

#[derive(Clone, Default)]
struct LoopEntry {
    tag: u16,
    conf: u8,   // Confidence (0-3)
    count: u16, // Current iteration count
    limit: u16, // Learned loop limit
    age: u8,    // Age counter for replacement policy
    dir: bool,  // The direction (Taken/NotTaken) that stays IN the loop
}
...
impl TagePredictor {
    ...
    fn index(&self, pc: u64, bank: usize) -> usize {
        ...
        let h = self.ghr & ((1 << len) - 1);
        let ph = self.phr & ((1 << len) - 1);
...
impl BranchPredictor for TagePredictor {
    ...
    fn update_branch(&mut self, pc: u64, taken: bool, target: Option<u64>) {
        ...
        if mispredicted {
            ...
            // Try to allocate in a bank larger than the provider
            if start_bank < num_banks {
                ...
                for i in start_bank..num_banks {
                    ...
                    if e.u == 0 {
                        e.tag = tag;
                        e.ctr = if taken { 0 } else { -1 }; // Weakly correct
                        e.u = 1; // Mark as useful
                        allocated = true;
                        break;
                    }
                }

                // If we couldn't allocate, decrement useful bits to make space for next time
                if !allocated {
                    for i in start_bank..num_banks {
                        let idx = self.index(pc, i);
