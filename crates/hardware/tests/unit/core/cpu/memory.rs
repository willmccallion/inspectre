...
fn test_cache_access_returns_latency() {
    ...
    let latency =
        cpu.simulate_memory_access(riscv_core::common::PhysAddr::new(paddr), AccessType::Read);
...
fn test_cache_access_instruction_fetch() {
    ...
    let latency =
        cpu.simulate_memory_access(riscv_core::common::PhysAddr::new(paddr), AccessType::Fetch);
...
fn test_cache_access_write() {
    ...
    let latency =
        cpu.simulate_memory_access(riscv_core::common::PhysAddr::new(paddr), AccessType::Write);
...
fn test_memory_access_with_different_access_types() {
    let mut cpu = create_test_cpu();
    let paddr = 0x8000_0000u64;
    let paddr_obj = riscv_core::common::PhysAddr::new(paddr);
...
fn test_cache_stats_updated() {
    ...
    cpu.simulate_memory_access(
        riscv_core::common::PhysAddr::new(0x8000_0000u64),
        AccessType::Fetch,
    );
...
fn test_l1_icache_enabled_hit_tracking() {
    ...
    let paddr = riscv_core::common::PhysAddr::new(0x8000_0000u64);
...
fn test_different_addresses_different_cache_lines() {
    ...
    for i in 0..10 {
        let paddr = riscv_core::common::PhysAddr::new(0x8000_0000u64 + i * 64);
        cpu.simulate_memory_access(paddr, AccessType::Read);
    }
