...
fn execute_vec_crypto(cpu: &mut Cpu, id: &RenameIssueEntry) -> Result<u64, Trap> {
    check_vill(id.inst, cpu.hart.csrs.vtype, cpu.soc.config.isa.vector.elen)?;
...
const fn build_ctx(cpu: &Cpu) -> VecExecCtx {
    let vtype = parse_vtype_with_elen(cpu.hart.csrs.vtype, cpu.soc.config.isa.vector.elen);
    VecExecCtx {
        sew: vtype.vsew,
        ...
        zvfh: cpu.soc.config.isa.vector.zvfh,
    }
}
